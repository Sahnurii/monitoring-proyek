@extends('layouts.app')

@section('content')
    <div class="container-fluid px-0 px-lg-3">
        <header
            class="d-flex flex-wrap flex-md-nowrap align-items-start align-items-md-center justify-content-between gap-3 mb-4">
            <div>
                <h1 class="fw-bold mb-1">Ubah Material</h1>
                <p class="text-muted mb-0">Perbarui informasi material dengan data terbaru.</p>
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('materials.show', $material) }}" class="btn btn-outline-secondary">
                    <i class="bi bi-arrow-left me-2"></i>Kembali
                </a>
            </div>
        </header>

        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-lg-5">
                @if ($errors->any())
                    <div class="alert alert-danger" role="alert">
                        <div class="fw-semibold mb-2">Terdapat kesalahan pada input Anda:</div>
                        <ul class="mb-0 ps-3">
                            @foreach ($errors->all() as $error)
                                <li>{{ $error }}</li>
                            @endforeach
                        </ul>
                    </div>
                @endif

                @if ($units->isEmpty())
                    <div class="alert alert-warning" role="alert">
                        Belum ada data satuan yang aktif. Silakan tambahkan satuan untuk memastikan data material valid.
                    </div>
                @endif

                <form action="{{ route('materials.update', $material) }}" method="POST" class="row g-4">
                    @csrf
                    @method('PUT')

                    <div class="col-md-6">
                        <label for="sku" class="form-label">SKU Material</label>
                        <input type="text" id="sku" class="form-control" value="{{ $material->sku }}" disabled>
                        <div class="form-text">SKU dibuat otomatis dan tidak dapat diubah.</div>
                    </div>

                    <div class="col-md-6">
                        <label for="name" class="form-label">Nama Material</label>
                        <input type="text" id="name" name="name" value="{{ old('name', $material->name) }}"
                            class="form-control @error('name') is-invalid @enderror" placeholder="Nama material" required>
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

                    <div class="col-md-6">
                        <label for="unit_id" class="form-label">Satuan</label>
                        <select id="unit_id" name="unit_id"
                            class="form-select @error('unit_id') is-invalid @enderror" required>
                            <option value="" disabled>Pilih satuan</option>
                            @foreach ($units as $unit)
                                <option value="{{ $unit->id }}"
                                    @selected(old('unit_id', $material->unit_id) == $unit->id)>
                                    {{ $unit->name }} ({{ $unit->symbol }})
                                </option>
                            @endforeach
                        </select>
                        @error('unit_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

                    <div class="col-md-6">
                        <label for="min_stock" class="form-label">Stok Minimal</label>
                        <div class="input-group">
                            <input type="number" id="min_stock" name="min_stock" min="0" step="0.01"
                                value="{{ old('min_stock', number_format($material->min_stock ?? 0, 2, '.', '')) }}"
                                class="form-control @error('min_stock') is-invalid @enderror" placeholder="0">
                            <span class="input-group-text">Satuan</span>
                        </div>
                        @error('min_stock')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                        <div class="form-text">Isi 0 jika tidak memiliki batas stok minimal.</div>
                    </div>

                    <div class="col-md-6">
                        <label for="unit_price" class="form-label">Harga Per Unit</label>
                        <div class="input-group">
                            <span class="input-group-text">Rp</span>
                            <input type="number" id="unit_price" name="unit_price" min="0" step="0.01"
                                value="{{ old('unit_price', number_format($material->unit_price ?? 0, 2, '.', '')) }}"
                                class="form-control @error('unit_price') is-invalid @enderror" placeholder="0" required>
                        </div>
                        @error('unit_price')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                        <div class="form-text">Gunakan harga dasar sebelum diskon atau pajak.</div>
                    </div>

                    <div class="col-12 d-flex justify-content-between gap-2">
                        <a href="{{ route('materials.show', $material) }}" class="btn btn-outline-secondary">Batal</a>
                        <button type="submit" class="btn btn-primary">Simpan Perubahan</button>
                    </div>
                </form>
            </div>
        </div>
    </div>
@endsection
